...
class Task(models.Model):
    ...
    estimated_minutes = models.PositiveIntegerField(default=0, help_text="Estimated time to complete in minutes")
    class Meta:
        ordering = ['-created_at']
    ...
    def total_time_spent(self):
        """Aggregates all completed focus sessions for this task."""
        return sum(session.duration_minutes for session in self.focus_sessions.filter(is_completed=True))
...
class FocusSession(models.Model):
    ...
    duration_minutes = models.PositiveIntegerField(default=25)  # Default Pomodoro block
