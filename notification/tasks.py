...
def generate_daily_overdue_digests():
    ...
    overdue_prefetch = Prefetch(
        'tasks',
        queryset=Task.objects.filter(completed=False, due_date__lt=now),
        to_attr='active_overdue_tasks'
    )
    users_with_overdue = User.objects.filter(
        tasks__completed=False, 
        tasks__due_date__lt=now
    ).distinct().prefetch_related(overdue_prefetch)

    for user in users_with_overdue:
        overdue_tasks = user.active_overdue_tasks 
        task_count = len(overdue_tasks)
        
        task_list_str = "\n".join([f"- {t.title} (Due: {t.due_date.strftime('%Y-%m-%d %H:%M')})" for t in overdue_tasks])
        email_msg = f"Hello {user.username},\n\nYou have {task_count} overdue tasks...\n\n{task_list_str}"
        
        Notification.objects.create(
            user=user, 
            notification_type='overdue', 
            message=f"Morning check-in: You have {task_count} overdue milestones."
        )
